...
const MOCK_USERS = {
  employee: { id: 'EMP001', email: 'alex.chan@example.com', name: 'Alex Chan', role: 'member' as const },
  employer: { id: 'BIZ001', email: 'contact@cloudflare.hk', name: 'Cloudflare HK', role: 'employer' as const },
};
export function HomePage() {
  ...
  const handleSignIn = () => {
    if (role === "employee") {
      if (employeeId === MOCK_USERS.employee.id && employeePassword) {
        login(MOCK_USERS.employee);
        toast.success(`Welcome back, ${MOCK_USERS.employee.name}!`);
      } else {
        toast.error("Invalid employee credentials.");
      }
    } else {
      if (employerId === MOCK_USERS.employer.id && employerPassword) {
        login(MOCK_USERS.employer);
        toast.success(`Welcome back, ${MOCK_USERS.employer.name}!`);
      } else {
        toast.error("Invalid employer credentials.");
      }
    }
  };
  return (
