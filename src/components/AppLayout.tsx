...
const employerNavItems = [
  ...
  { to: "/employer/contributions", icon: Wallet, label: "Contributions" },
  { to: "/employer/reports", icon: FileText, label: "Reports" },
    ...
    <nav className="grid items-start px-2 text-sm font-medium lg:px-4">
      {navItems.map(({ to, icon: Icon, label }) => (
        <NavLink
          key={to}
          to={to}
          end // Use `end` prop for exact matching on dashboard routes
          className={({ isActive }) =>
            cn(
    ...
    <div className="grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]">
      <div className="hidden border-r bg-muted/40 md:block">
        <div className="flex h-full max-h-screen flex-col gap-2">
          <div className="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
            <NavLink to={user.role === 'employer' ? '/employer/dashboard' : '/dashboard'} className="flex items-center gap-2 font-semibold">
              <Package className="h-6 w-6 text-blue-600" />
              <span className="">ProvidentEdge</span>
            </NavLink>
          </div>
