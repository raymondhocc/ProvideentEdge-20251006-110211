...
import { UpdatePortfolioRequestSchema } from "@shared/schemas";
export function userRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  app.post('/api/member/portfolio', async (c) => {
    ...
    const validation = UpdatePortfolioRequestSchema.safeParse(body);
    if (!validation.success) {
      return bad(c, validation.error.flatten().fieldErrors.allocation?.[0] || 'Invalid allocation data');
    }
    const { allocation } = validation.data;
    const member = new MemberEntity(c.env, MOCK_MEMBER_SUMMARY.id);
    if (!await member.exists()) return notFound(c, 'member not found');
    await member.updatePortfolio(allocation);
  ...
  app.get('/api/employer/profile', async (c) => {
    const employer = new EmployerEntity(c.env, MOCK_EMPLOYER_SUMMARY.id);
    if (!await employer.exists()) return notFound(c, 'employer not found');
    const data = await employer.getState();
    return ok(c, data.profile);
  });
}